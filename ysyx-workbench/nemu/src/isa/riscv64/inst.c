#include "local-include/reg.h"
#include <cpu/cpu.h>
#include <cpu/ifetch.h>
#include <cpu/decode.h>
#include <elf.h>

/* elf file parsing variables */
extern char *StringTable;
extern char *strtab;

extern int fd;
extern uint8_t sym_shdr;
extern uint8_t str_shdr;

extern uint8_t *elf_mem;
extern struct stat st;
/* the elf header */
extern Elf64_Ehdr *ehdr;
/* Section header */
extern Elf64_Shdr *shdr;
/* Symbol table */
extern Elf64_Sym *sym;

#define R(i) gpr(i)
#define Mr vaddr_read
#define Mw vaddr_write

enum {
  TYPE_I, TYPE_U, TYPE_S,
  TYPE_N, TYPE_R, TYPE_B,TYPE_J,// none
};

#define src1R(n) do { *src1 = R(n); } while (0)
#define src2R(n) do { *src2 = R(n); } while (0)
#define destR(n) do { *dest = n; } while (0)
#define src1I(i) do { *src1 = i; } while (0)
#define src2I(i) do { *src2 = i; } while (0)
#define destI(i) do { *dest = i; } while (0)

static word_t immI(uint32_t i) { return SEXT(BITS(i, 31, 20), 12); }
static word_t immU(uint32_t i) { return SEXT(BITS(i, 31, 12), 20) << 12; }
static word_t immS(uint32_t i) { return (SEXT((BITS(i, 31, 25) << 5) | BITS(i, 11, 7),12)); }
static word_t immJ(uint32_t i) { return (SEXT((((((((BIT(i, 31) << 8) | BITS(i, 19, 12)) << 1 )| BIT(i,20)) << 10) |BITS(i, 30, 21)) << 1),21)); }
static word_t immB(uint32_t i) { return (SEXT((((((BIT(i, 31) << 1) | BIT(i,7)) << 6 | BITS(i, 30, 25)) << 4 | BITS(i, 11, 8)) << 1),13)); }

//static word_t immJ(uint32_t i) { return ((((((SEXT(BITS(i, 31, 30), 1) << 8) | BITS(i, 19, 12)) << 1 )| BITS(i, 21, 20)) << 10 )|BITS(i, 30, 21)) << 1; }
//static word_t immB(uint32_t i) { return ((((SEXT(BITS(i, 31, 30), 1) << 1) | BITS(i, 8, 7)) << 5 | BITS(i, 30, 25)) << 4 | BITS(i, 11, 8)) << 1; }

static void decode_operand(Decode *s, word_t *dest, word_t *src1, word_t *src2, int type) {
  uint32_t i = s->isa.inst.val;
  int rd  = BITS(i, 11, 7);
  int rs1 = BITS(i, 19, 15);
  int rs2 = BITS(i, 24, 20);
  destR(rd);

  switch (type) {
    
    case TYPE_I: src1R(rs1);     src2I(immI(i)); break;
    case TYPE_U: src1I(immU(i)); break;
    case TYPE_S: destI(immS(i)); src1R(rs1); src2R(rs2); break;
    case TYPE_J: src1I(immJ(i)); break;
    case TYPE_B: destI(immB(i)); src1R(rs1); src2R(rs2); break;
    case TYPE_R: src1R(rs1); src2R(rs2); break;
  }
}

static int decode_exec(Decode *s) {
  word_t dest = 0, src1 = 0, src2 = 0;
  word_t temp = 0;
  s->dnpc = s->snpc;

#define INSTPAT_INST(s) ((s)->isa.inst.val)
#define INSTPAT_MATCH(s, name, type, ... /* body */ ) { \
  decode_operand(s, &dest, &src1, &src2, concat(TYPE_, type)); \
  __VA_ARGS__ ; \
}


  INSTPAT_START();
  /* Control Transfer Instruction */
  INSTPAT("??????? ????? ????? 000 ????? 11000 11", beq    , B, s->dnpc = src1==src2 ? s->pc + dest : s->pc+4);
  INSTPAT("??????? ????? ????? 001 ????? 11000 11", bne    , B, s->dnpc = src1!=src2 ? s->pc + dest : s->pc+4);
  INSTPAT("??????? ????? ????? 101 ????? 11000 11", bge    , B, s->dnpc = (sword_t)src1>=(sword_t)src2 ? s->pc + dest : s->pc+4);
  INSTPAT("??????? ????? ????? 111 ????? 11000 11", bgeu   , B, s->dnpc = src1>=src2 ? s->pc + dest : s->pc+4);
  INSTPAT("??????? ????? ????? 100 ????? 11000 11", blt    , B, s->dnpc = (sword_t)src1 < (sword_t)src2 ? s->pc + dest : s->pc+4);
  INSTPAT("??????? ????? ????? 110 ????? 11000 11", bltu   , B, s->dnpc = src1 < src2 ? s->pc + dest : s->pc+4);

  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, s->dnpc = s->pc + src1,R(dest)= s->pc + 4);
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, temp = s->pc+4,s->dnpc = (src2+src1)&~1,R(dest) = temp); //ret
  //INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, temp = s->pc+4,s->dnpc = (src2+src1)&~1,R(dest) = temp); //ret

  /* Arithmetic Instruction */
  INSTPAT("??????? ????? ????? ??? ????? 00101 11", auipc  , U, R(dest) = src1 + s->pc);
  INSTPAT("??????? ????? ????? ??? ????? 01101 11", lui    , U, R(dest) = src1);
  
  INSTPAT("??????? ????? ????? 000 ????? 00100 11", addi   , I, R(dest) = src1 + src2);
  INSTPAT("0000000 ????? ????? 000 ????? 01100 11", add    , R, R(dest) = src1 + src2);
  INSTPAT("0100000 ????? ????? 000 ????? 01100 11", sub    , R, R(dest) = src1 - src2);

  INSTPAT("??????? ????? ????? 011 ????? 00100 11", sltiu  , I, R(dest) = src1 < src2 ? 1 : 0);          //seqz
  INSTPAT("0000000 ????? ????? 011 ????? 01100 11", sltu   , R, R(dest) = src1 < src2 ? 1 : 0); //snez
  INSTPAT("0000000 ????? ????? 010 ????? 01100 11", slt    , R, R(dest) = (sword_t)src1 < (sword_t)src2 ? 1 : 0); 

  /* Arithmetic Instructions */
  INSTPAT("??????? ????? ????? 111 ????? 00100 11", andi   , I, R(dest) = src1 & src2);
  INSTPAT("0000000 ????? ????? 111 ????? 01100 11", and    , R, R(dest) = src1 & src2);
  INSTPAT("??????? ????? ????? 100 ????? 00100 11", xoir   , I, R(dest) = src1 ^ src2);
  INSTPAT("0000000 ????? ????? 110 ????? 01100 11", or     , R, R(dest) = src1 | src2);
  INSTPAT("??????? ????? ????? 110 ????? 00100 11", ori    , I, R(dest) = src1 | src2);
  INSTPAT("0000000 ????? ????? 100 ????? 01100 11", xor    , R, R(dest) = src1 ^ src2);

 
  /* Integer Computational shift Instructions */
  INSTPAT("010000? ????? ????? 101 ????? 00100 11", srai   , I, R(dest) = ((sword_t) src1 >> BITS(src2,5,0) )); //有符号数右移高位补符号位
  INSTPAT("0100000 ????? ????? 101 ????? 00110 11", sraiw  , I, R(dest) = SEXT(((int32_t)BITS(src1,31,0) >> BITS(src2,5,0)),32)); //IW [4:0]
  INSTPAT("0000000 ????? ????? 001 ????? 01110 11", sllw   , R, R(dest) = SEXT((BITS((BITS(src1,31,0) << BITS(src2,4,0)),31,0)),32));
  INSTPAT("000000? ????? ????? 001 ????? 00100 11", slli   , I, R(dest) = src1 << BITS(src2,5,0));
  INSTPAT("000000? ????? ????? 101 ????? 00100 11", srli   , I, R(dest) = src1 >> BITS(src2,5,0));
  INSTPAT("0000000 ????? ????? 001 ????? 00110 11", slliw  , I, R(dest) = SEXT((BITS(src1,31,0) << BITS(src2,5,0)),32));
  INSTPAT("0000000 ????? ????? 101 ????? 00110 11", srliw  , I, R(dest) = SEXT((BITS(src1,31,0) >> BITS(src2,5,0)),32));
  INSTPAT("0100000 ????? ????? 101 ????? 01110 11", sraw   , R, R(dest) = SEXT((BITS(((int32_t)BITS(src1,31,0) >> BITS(src2,4,0)),31,0)),32));
  INSTPAT("0000000 ????? ????? 101 ????? 01110 11", srlw   , R, R(dest) = SEXT((BITS((BITS(src1,31,0) >> BITS(src2,4,0)),31,0)),32));
  INSTPAT("0000000 ????? ????? 001 ????? 01100 11", sll    , R, R(dest) = src1 << BITS(src2,5,0));



  /* Load and Store Instructions */  
  INSTPAT("??????? ????? ????? 011 ????? 00000 11", ld     , I, R(dest) = Mr(src1 + src2, 8));
  INSTPAT("??????? ????? ????? 010 ????? 00000 11", lw     , I, R(dest) = SEXT(Mr(src1 + src2, 4),32));
  INSTPAT("??????? ????? ????? 110 ????? 00000 11", lwu    , I, R(dest) = ZEXT(Mr(src1 + src2, 4),32));
  INSTPAT("??????? ????? ????? 001 ????? 00000 11", lh     , I, R(dest) = SEXT(Mr(src1 + src2, 2),16));
  INSTPAT("??????? ????? ????? 101 ????? 00000 11", lhu    , I, R(dest) = ZEXT(Mr(src1 + src2, 2),16));
  INSTPAT("??????? ????? ????? 100 ????? 00000 11", lbu    , I, R(dest) = ZEXT(Mr(src1 + src2, 1),8)); 
  INSTPAT("??????? ????? ????? 000 ????? 00000 11", lb     , I, R(dest) = SEXT(Mr(src1 + src2, 1),8));


  INSTPAT("??????? ????? ????? 011 ????? 01000 11", sd     , S, Mw(src1 + dest, 8, src2));
  INSTPAT("??????? ????? ????? 001 ????? 01000 11", sh     , S, Mw(src1 + dest, 2, BITS(src2,15,0)));
  INSTPAT("??????? ????? ????? 000 ????? 01000 11", sb     , S, Mw(src1 + dest, 1, BITS(src2,7,0)));
  INSTPAT("??????? ????? ????? 010 ????? 01000 11", sw     , S, Mw(src1 + dest, 4, BITS(src2,31,0)));

  /*RV64M Multiplication Operations*/
  INSTPAT("0100000 ????? ????? 000 ????? 01110 11", subw   , R, R(dest) = SEXT(BITS((src1 - src2),31,0),32));
  INSTPAT("??????? ????? ????? 000 ????? 00110 11", addiw  , I, R(dest) = SEXT(BITS((src1 + src2),31,0),32));
  INSTPAT("0000000 ????? ????? 000 ????? 01110 11", addw   , R, R(dest) = SEXT(BITS((src1 + src2),31,0),32));
  INSTPAT("0000001 ????? ????? 000 ????? 01100 11", mul    , R, R(dest) = src1 * src2);
  INSTPAT("0000001 ????? ????? 000 ????? 01110 11", mulw   , R, R(dest) = SEXT((BITS((BITS(src1,31,0) * BITS(src2,31,0)),31,0)),32));
  /*RV64M Divsision Operations*/
  INSTPAT("0000001 ????? ????? 100 ????? 01110 11", divw   , R, R(dest) = SEXT((BITS((int32_t)(BITS(src1,31,0) / (int32_t)BITS(src2,31,0)),31,0)),32));
  INSTPAT("0000001 ????? ????? 111 ????? 01110 11", remuw  , R, R(dest) = SEXT((BITS((BITS(src1,31,0) % BITS(src2,31,0)),31,0)),32));
  INSTPAT("0000001 ????? ????? 101 ????? 01110 11", divuw  , R, R(dest) = SEXT((BITS((BITS(src1,31,0) / BITS(src2,31,0)),31,0)),32));
  INSTPAT("0000001 ????? ????? 110 ????? 01110 11", remw   , R, R(dest) = SEXT((BITS((int32_t)(BITS(src1,31,0) % (int32_t)BITS(src2,31,0)),31,0)),32));
  INSTPAT("0000001 ????? ????? 111 ????? 01100 11", remu   , R, R(dest) = src1 % src2);
  INSTPAT("0000001 ????? ????? 101 ????? 01100 11", divu   , R, R(dest) = src1 / src2);


  /* Environment Call and Break Instruction */
  INSTPAT("0000000 00001 00000 000 00000 11100 11", ebreak , N, NEMUTRAP(s->pc, R(10))); // R(10) is $a0
  INSTPAT("??????? ????? ????? ??? ????? ????? ??", inv    , N, INV(s->pc));   //若前面所有的模式匹配规则都无法成功匹配，则该指令视为非法指令
  INSTPAT_END();

  R(0) = 0; // reset $zero to 0

  #ifdef CONFIG_FTRACE 
  char *f = s->elfbuf;
  int len = 0;
  uint8_t opcode = BITS(s->isa.inst.val, 6, 0);
  uint8_t rs1 = BITS(s->isa.inst.val, 19, 15);
  int j ;
    /* store current pc to dest(ra)#1 regisiter  chapter 2.7 */
    /* s->dnpc represent fucntin entry adderss */
    /* jalr call function  ||  jal call function */
  if((opcode==0b01100111&&dest!=0)||(opcode == 0b01101111&&dest==1))
  {
    //printf("rd=%d,rs1=%d\n",dest,rs1);          
    for (j = 1; j < (shdr[sym_shdr].sh_size) / (shdr[sym_shdr].sh_entsize); j++)
    {
      if ((sym[j].st_info & 0x0f) == STT_FUNC)
      {
        if((s->dnpc >= sym[j].st_value)&&(s->dnpc < sym[j].st_value+sym[j].st_size))
        {
          len = sprintf(f,"0x%08lx\t",s->pc);
          len += sprintf(f+len,"call [%s@%08lx]",strtab + sym[j].st_name,sym[j].st_value);
          printf("%s\n",f);          
          //printf("%s\n",strtab + sym[j].st_name);
        }
      }
    }
  }
    /* jalr ret function  ||  jal ret function */
  if((opcode==0b01100111&&dest==0&&rs1==1)||(opcode == 0b01101111&&dest==0))
  {
    for (j = 1; j < (shdr[sym_shdr].sh_size) / (shdr[sym_shdr].sh_entsize); j++)
    {
      if ((sym[j].st_info & 0x0f) == STT_FUNC)
      {
        if((s->pc >= sym[j].st_value)&&(s->pc < sym[j].st_value+sym[j].st_size))
        {
          len = sprintf(f,"0x%08lx\t",s->pc);         
          len += sprintf(f+len,"ret [%s]",strtab + sym[j].st_name);
          printf("%s\n",f);          
          //printf("%s\n",strtab + sym[j].st_name);
        }
      }
    }
  }
  #endif

  return 0;
}

int isa_exec_once(Decode *s) {
  s->isa.inst.val = inst_fetch(&s->snpc, 4);
  return decode_exec(s);
}
